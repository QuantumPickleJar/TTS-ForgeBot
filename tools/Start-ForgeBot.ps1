...
# Forge's lazy loader cannot resolve all CopyFaceFrom dependencies used by imported TTS decks.
# Keep the complete card database loaded so Forge remains authoritative for every supplied card.
$lazyCardScriptsOption = ''
...
if ($LazyCardScripts) { Write-Warning 'Ignoring -LazyCardScripts: it can omit imported cards that use CopyFaceFrom dependencies.' }
