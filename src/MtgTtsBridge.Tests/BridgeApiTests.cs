...
public sealed class BridgeApiTests
{
    ...
    public async Task ChoiceWireShapeFromTts_BindsEveryProtocolIdentityField()
    {
        ...
        using var client = factory.CreateClient();
        var sessionId = await StartSessionAsync(client);
        var json = $$"""{"sessionId":"{{sessionId}}","decisionId":"decision-1-main","actionId":"play_mountain","requestId":"runtime-1-choice-1","clientRuntimeId":"runtime-1","clientRevision":"2026-08-25-f2b-v3","source":"pass_button"}""";
