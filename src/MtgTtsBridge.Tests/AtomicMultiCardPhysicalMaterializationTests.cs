...
public sealed class AtomicMultiCardPhysicalMaterializationTests
{
    ...
    [Fact]
    public void VirtualAbilityProvenanceUsesMappedPhysicalPermanentForReadiness()
    {
        var lua = NewProbe();
        ExecuteProbe(lua, @"
            BridgeTestInitAtomicHarness()
            ...
            local battlefieldTrio = BridgeTestCreateCard('forge:activation-session:32', 'Harmonized Trio', 'trio-guid')
            battlefieldTrio._inLibrary = false
            local duplicateName = BridgeTestCreateCard('forge:activation-session:31', 'Harmonized Trio', 'other-trio-guid')
            duplicateName._inLibrary = false
            BridgeRecordLooseCardIdentity('forge:activation-session:32', 'trio-guid', 'forge-player-1', 'battlefield')
            BridgeRecordLooseCardIdentity('forge:activation-session:31', 'other-trio-guid', 'forge-player-1', 'graveyard')

            -- This is the follow-up provenance shape that matters: the
            -- logical ability/stack identity is virtual, while the exact
            -- permanent that supplied it is still the physical source.
            BridgeState.authoritativeObjectByInstanceId['forge:activation-session:stack:82'] = {
                cardInstanceId='forge:activation-session:stack:82', objectId='forge-object:82',
                isVirtual=true, materializationPolicy='virtual-stack',
                zone='stack', seatId='forge-player-1'
            }
            BridgeState.authoritativeObjectByInstanceId['forge:activation-session:32'] = {
                cardInstanceId='forge:activation-session:32', objectId='forge-object:32',
                isVirtual=false, materializationPolicy='physical',
                zone='battlefield', seatId='forge-player-1'
            }
            local followup = {
                actionId='forge-tui-42-choice-1', type='activate_ability',
                actionKind='activate_ability', cardInstanceId='forge:activation-session:stack:82',
                sourceCardInstanceId='forge:activation-session:32', entityCardInstanceId='forge:activation-session:32',
                sourceZone='stack', cardIdentity='Harmonized Trio',
                provenance={sourceCardInstanceId='forge:activation-session:32', sourceZone='stack'}
            }
            local decision = {
                decisionId='forge-tui-42', kind='main_priority',
                seatId='forge-player-1', actions={followup}
            }
            recoverySchedules = 0
            visibleErrors = 0
            BridgeScheduleSnapshotReconcile = function(...) recoverySchedules = recoverySchedules + 1 end
            ...
            BridgeState.ui = {mounted=true, actionRows={}, gameLogVisible=false,
                fastForwardActive=false, autoAdvanceMode='NORMAL', autoPassEmpty=false}
            BridgeState.lastDecision = decision

            tapApplied, tapDelay, tapError = BridgeApplyAuthoritativeEvent({
                sequence=411, kind='tap_changed', seatId='forge-player-1',
                cardInstanceId='forge:activation-session:32',
                cardName='Harmonized Trio', sourceZone='battlefield',
                destinationZone='battlefield', tapped=true, forgeSequence=81
            })
            exact = BridgeActionExactPhysicalInstanceId(followup)
            expectedZone = BridgeActionExpectedPhysicalSourceZone(followup)
            ready, reason = BridgeDecisionPhysicalMappingsReady(decision)
            resolved, resolveError = BridgeResolveExactActionPhysical(decision, followup)
            BridgeRenderDecision(decision, true)
            renderedKey = BridgeState.renderedDecisionPresentationKey
            resolvedGuid = resolved and resolved.guid or 'nil'
            trioGuid = BridgeState.physicalByInstanceId['forge:activation-session:32']
            trioInverse = BridgeState.physicalInstanceIdByGuid['trio-guid']
            trioZone = BridgeState.physicalZoneByGuid['trio-guid']
        ");

        Assert.Equal("forge:activation-session:32", lua.Globals.Get("exact").String);
        Assert.Equal("battlefield", lua.Globals.Get("expectedZone").String);
        Assert.True(lua.Globals.Get("tapApplied").Boolean, lua.Globals.Get("tapError").ToPrintString());
        Assert.True(lua.Globals.Get("ready").Boolean, lua.Globals.Get("reason").ToPrintString());
        Assert.False(lua.Globals.Get("renderedKey").IsNil());
        Assert.True(lua.Globals.Get("resolvedGuid").String == "trio-guid",
            lua.Globals.Get("resolveError").ToPrintString());
        Assert.Equal("trio-guid", lua.Globals.Get("trioGuid").String);
        Assert.Equal("forge:activation-session:32", lua.Globals.Get("trioInverse").String);
        Assert.Equal("battlefield", lua.Globals.Get("trioZone").String);
        Assert.Equal(0, lua.Globals.Get("recoverySchedules").Number);
        Assert.Equal(0, lua.Globals.Get("visibleErrors").Number);
    }
    ...
    [Fact]
    public void DelveContainedSixCardExtractionDrainsDeckBeforeSupplierMill()
    {
        var lua = NewProbe();
        ExecuteProbe(lua, @"
            BridgeTestInitAtomicHarness()
            BridgeState.eventSessionId = 'b00d31c77eca400caaa1fa389a7eca9c'
            BridgeState.eventSessionGeneration = 2
            BridgeState.lastAppliedEventSequence = 135
            local session = BridgeState.eventSessionId
            local d11 = 'forge:' .. session .. ':11'
            local d34 = 'forge:' .. session .. ':34'
            local d5 = 'forge:' .. session .. ':5'
            local d29 = 'forge:' .. session .. ':29'
            local d28 = 'forge:' .. session .. ':28'
            local d25 = 'forge:' .. session .. ':25'
            local d19 = 'forge:' .. session .. ':19'
            local pendingWaitCallbacks = {}
            local function queueWaitCallback(callback)
                if callback ~= nil then table.insert(pendingWaitCallbacks, callback) end
            end
            BridgeWaitFrames = function(callback, frames) queueWaitCallback(callback) end
            BridgeWaitTime = function(callback, delay) queueWaitCallback(callback) end
            bridgeTest.collapseGraveyardAtOne = true
            function BridgeTestDrainWaitCallbacks()
                local guard = 0
                while pendingWaitCallbacks[1] ~= nil do
                    guard = guard + 1
                    if guard > 200 then error('graveyard regression wait callback livelock') end
                    local callback = pendingWaitCallbacks[1]
                    local count = BridgeTestArrayLength(pendingWaitCallbacks)
                    for shift = 1, count - 1 do pendingWaitCallbacks[shift] = pendingWaitCallbacks[shift + 1] end
            ...
            BridgeGetLiveObjectByGuid = function(guid)
                local object = rawGetLive(guid)
                -- A contained native Deck entry is not a top-level Card.
                if object ~= nil and object.tag == 'Card' and object._inDeck == true then return nil end
                return object
            end
            bridgeTest.graveyardContainer = bridgeTest.graveyardDeck
            local function seedDelveCard(instanceId, cardName, guid)
                BridgeState.cardNameByInstanceId[instanceId] = cardName
                ...
                BridgeRecordContainedCardIdentity(instanceId, bridgeTest.graveyardDeck.getGUID(),
                    guid, 'forge-player-1', 'graveyard', cardName)
            end
            seedDelveCard(d11, 'Island', 'delve-island')
            seedDelveCard(d34, 'Thought Scour', 'delve-scour')
            seedDelveCard(d5, 'Murderous Cut', 'delve-cut')
            seedDelveCard(d29, 'Mental Note', 'delve-note-a')
            seedDelveCard(d28, 'Mental Note', 'delve-note-b')
            seedDelveCard(d25, 'Ashiok, Nightmare Weaver', 'delve-ashiok')
            BridgeState.cardNameByInstanceId[d19] = 'Swamp'
            local tappedSwamp = BridgeTestCreateCard(d19, 'Swamp', 'delve-tapped-swamp')
            tappedSwamp._inLibrary = false
            BridgeRecordLooseCardIdentity(d19, 'delve-tapped-swamp', 'forge-player-1', 'battlefield')
            BridgeTestSetEventQueue(
                {sequence=136, kind='card_moved', seatId='forge-player-1', sourceZone='graveyard', destinationZone='exile', cardInstanceId=d11, cardName='Island', forgeSequence=35},
                {sequence=137, kind='tap_changed', seatId='forge-player-1', sourceZone='battlefield', destinationZone='battlefield', cardInstanceId=d19, cardName='Swamp', tapped=true, forgeSequence=35},
                {sequence=138, kind='card_moved', seatId='forge-player-1', sourceZone='graveyard', destinationZone='exile', cardInstanceId=d34, cardName='Thought Scour', forgeSequence=35},
                {sequence=139, kind='card_moved', seatId='forge-player-1', sourceZone='graveyard', destinationZone='exile', cardInstanceId=d5, cardName='Murderous Cut', forgeSequence=35},
                {sequence=140, kind='card_moved', seatId='forge-player-1', sourceZone='graveyard', destinationZone='exile', cardInstanceId=d29, cardName='Mental Note', forgeSequence=35},
                {sequence=141, kind='card_moved', seatId='forge-player-1', sourceZone='graveyard', destinationZone='exile', cardInstanceId=d28, cardName='Mental Note', forgeSequence=35},
                {sequence=142, kind='card_moved', seatId='forge-player-1', sourceZone='graveyard', destinationZone='exile', cardInstanceId=d25, cardName='Ashiok, Nightmare Weaver', forgeSequence=35}
            )
            BridgeProcessEventQueue()
            extractionQueuedBeforeFlush = BridgeTestArrayLength(BridgeState.graveyardExtractionQueueBySeatId['forge-player-1'] or {})
            extractionStartedBeforeFlush = BridgeState.graveyardExtractionActiveBySeatId['forge-player-1'] == true
            while BridgeTestFlushNextGraveyardTake() do end
            BridgeTestDrainWaitCallbacks()
            noStaleDelveCards = true
            noStaleDelveCards = BridgeState.physicalContainerByInstanceId[d11] == nil
                and BridgeState.physicalContainerByInstanceId[d34] == nil
                and BridgeState.physicalContainerByInstanceId[d5] == nil
                and BridgeState.physicalContainerByInstanceId[d29] == nil
                and BridgeState.physicalContainerByInstanceId[d28] == nil
                and BridgeState.physicalContainerByInstanceId[d25] == nil
                and BridgeState.physicalZoneByGuid['delve-island'] ~= 'graveyard'
                and BridgeState.physicalZoneByGuid['delve-scour'] ~= 'graveyard'
                and BridgeState.physicalZoneByGuid['delve-cut'] ~= 'graveyard'
                and BridgeState.physicalZoneByGuid['delve-note-a'] ~= 'graveyard'
                and BridgeState.physicalZoneByGuid['delve-note-b'] ~= 'graveyard'
                and BridgeState.physicalZoneByGuid['delve-ashiok'] ~= 'graveyard'
            delveApplied = BridgeState.lastAppliedEventSequence
            delveCommitCount = BridgeTestCountLogToken('MUTATION_COMMIT')
            delveAbortCount = BridgeTestCountLogToken('MUTATION_ABORT')

            -- The real capture has unrelated committed events 143..198
            -- between the Delve payment and Supplier's ETB. Start the next
            -- fixture boundary at the same authoritative cursor.
            BridgeState.lastAppliedEventSequence = 198
            local s36 = 'forge:' .. session .. ':36'
            local s22 = 'forge:' .. session .. ':22'
            local s35 = 'forge:' .. session .. ':35'
            local extractionCards = {
                BridgeTestCreateCard(s36, 'Baleful Strix', 'supplier-baleful-strix'),
                BridgeTestCreateCard(s22, 'Swamp', 'supplier-swamp'),
                BridgeTestCreateCard(s35, 'Thought Scour', 'supplier-thought-scour')
            }
            BridgeState.cardNameByInstanceId[s36] = 'Baleful Strix'
            BridgeState.cardNameByInstanceId[s22] = 'Swamp'
            BridgeState.cardNameByInstanceId[s35] = 'Thought Scour'
            BridgeTestQueueExtractionCards(extractionCards)
            BridgeTestSetEventQueue(
                {sequence=199, kind='card_moved', seatId='forge-player-1', sourceZone='library', destinationZone='graveyard', cardInstanceId=s36, cardName='Baleful Strix', forgeSequence=52},
                {sequence=200, kind='card_moved', seatId='forge-player-1', sourceZone='library', destinationZone='graveyard', cardInstanceId=s22, cardName='Swamp', forgeSequence=52},
                {sequence=201, kind='card_moved', seatId='forge-player-1', sourceZone='library', destinationZone='graveyard', cardInstanceId=s35, cardName='Thought Scour', forgeSequence=52}
            )
            BridgeProcessEventQueue()
            BridgeTestDrainWaitCallbacks()
            local graveyardLedger = BridgeZoneLedger('forge-player-1', 'graveyard') or {}
            local exactSupplierCount = 0
            local exactSupplierIds = {}
            exactSupplierCount = BridgeTestGraveyardInstanceCount(s36)
                + BridgeTestGraveyardInstanceCount(s22)
                + BridgeTestGraveyardInstanceCount(s35)
            exactSupplierIds[s36] = BridgeTestGraveyardInstanceCount(s36) == 1
            exactSupplierIds[s22] = BridgeTestGraveyardInstanceCount(s22) == 1
            exactSupplierIds[s35] = BridgeTestGraveyardInstanceCount(s35) == 1
            supplierApplied = BridgeState.lastAppliedEventSequence
            supplierNativeCount = BridgeTestArrayLength(bridgeTest.graveyardDeck.entries or {})
            supplierLedgerCount = BridgeTestArrayLength(graveyardLedger)
            supplierCommitCount = BridgeTestCountLogToken('MUTATION_COMMIT')
            supplierAbortCount = BridgeTestCountLogToken('MUTATION_ABORT')
            supplierRebindFailure = tostring(BridgeState.lastGraveyardRebindFailure)
            noDuplicateSupplierGuids = BridgeTestDuplicateGraveyardInstanceCount() == 0
            allSupplierIdsPreserved = exactSupplierIds[s36] == true
                and exactSupplierIds[s22] == true and exactSupplierIds[s35] == true

            -- The capture's next Forge decision is still main priority.  The
            -- Trio in hand must remain a separately exact physical action;
            -- the preceding graveyard batch must not force recovery or a
            -- same-name substitution.
            local trioId = 'forge:' .. session .. ':32'
            local trio = BridgeTestCreateCard(trioId, 'Harmonized Trio', 'trio-in-hand-guid')
            trio._inLibrary = false
            trio._inHand = true
            BridgeState.cardNameByInstanceId[trioId] = 'Harmonized Trio'
            BridgeRecordLooseCardIdentity(trioId, 'trio-in-hand-guid', 'forge-player-1', 'hand')
            BridgeState.authoritativeObjectByInstanceId[trioId] = {
                cardInstanceId=trioId, objectId='forge-object:32',
                isVirtual=false, materializationPolicy='physical',
                zone='hand', seatId='forge-player-1'
            }
            local trioAction = {
                actionId='forge-tui-19-choice-1', type='cast_spell',
                actionKind='cast_spell', cardInstanceId=trioId,
                sourceCardInstanceId=trioId, entityCardInstanceId=trioId,
                sourceZone='hand', castMode='normal',
                cardIdentity='Harmonized Trio', sourceCardName='Harmonized Trio'
            }
            local nextDecision = {
                decisionId='forge-tui-19', kind='main_priority',
                seatId='forge-player-1', eventCursor=201,
                actions={
                    {actionId='forge-tui-19-choice-0', type='pass_priority', actionKind='pass_priority'},
                    trioAction
                }
            }
            trioExactInstanceId = BridgeActionExactPhysicalInstanceId(trioAction)
            trioReady, trioReason = BridgeDecisionPhysicalMappingsReady(nextDecision)
            trioResolved, trioResolveError = BridgeResolveExactActionPhysical(nextDecision, trioAction)
            trioResolvedGuid = trioResolved and trioResolved.guid or 'nil'
        ");

        Assert.True(lua.Globals.Get("extractionStartedBeforeFlush").Boolean, CapturedLogsTail(lua));
        Assert.Equal(6, lua.Globals.Get("extractionQueuedBeforeFlush").Number);
        Assert.True(lua.Globals.Get("noStaleDelveCards").Boolean,
            $"delveApplied={lua.Globals.Get("delveApplied").ToPrintString()} logs={CapturedLogsTail(lua)}");
        Assert.Equal(142, lua.Globals.Get("delveApplied").Number);
        Assert.Equal(0, lua.Globals.Get("delveAbortCount").Number);
        Assert.Equal(1, lua.Globals.Get("delveCommitCount").Number);
        Assert.True(lua.Globals.Get("supplierApplied").Number == 201, CapturedLogsTail(lua));
        Assert.True(lua.Globals.Get("supplierNativeCount").Number == 3, CapturedLogsTail(lua));
        Assert.Equal(3, lua.Globals.Get("supplierLedgerCount").Number);
        Assert.True(lua.Globals.Get("allSupplierIdsPreserved").Boolean);
        Assert.True(lua.Globals.Get("noDuplicateSupplierGuids").Boolean);
        Assert.Equal(2, lua.Globals.Get("supplierCommitCount").Number);
        Assert.True(lua.Globals.Get("supplierAbortCount").Number == 0,
            $"rebind={lua.Globals.Get("supplierRebindFailure").String} logs={CapturedLogsTail(lua)}");
        Assert.Equal("forge:b00d31c77eca400caaa1fa389a7eca9c:32",
            lua.Globals.Get("trioExactInstanceId").String);
        Assert.True(lua.Globals.Get("trioReady").Boolean,
            $"reason={lua.Globals.Get("trioReason").ToPrintString()} logs={CapturedLogsTail(lua)}");
        Assert.True(lua.Globals.Get("trioResolvedGuid").String == "trio-in-hand-guid",
            lua.Globals.Get("trioResolveError").ToPrintString());
    }

    [Fact]
    public void FourCardGraveyardDrainReobservesDeckCollapseBeforeFinalExactExtraction()
    {
        var lua = NewProbe();
        ExecuteProbe(lua, @"
            BridgeTestInitAtomicHarness()
            BridgeState.eventSessionId = '2e629fa847ea4b1f9e3e1d61a75eeb31'
            BridgeState.eventSessionGeneration = 2
            BridgeState.physicalTransactionGeneration = 2
            BridgeState.lastAppliedEventSequence = 164
            BridgeState.desyncLatched = false
            bridgeTest.collapseGraveyardAtOne = true
            bridgeTest.collapsedGraveyardLooseVisible = false

            local i6 = 'forge:2e629fa847ea4b1f9e3e1d61a75eeb31:6'
            local i24 = 'forge:2e629fa847ea4b1f9e3e1d61a75eeb31:24'
            local i2 = 'forge:2e629fa847ea4b1f9e3e1d61a75eeb31:2'
            local i31 = 'forge:2e629fa847ea4b1f9e3e1d61a75eeb31:31'
            local function seed(instanceId, cardName, guid)
                BridgeState.cardNameByInstanceId[instanceId] = cardName
                local card = BridgeTestCreateCard(instanceId, cardName, guid)
                card._inDeck = true
                card._inLibrary = false
            ...
            seed(i6, 'Murderous Cut', 'grave-6')
            seed(i24, 'Swamp', 'grave-24')
            seed(i2, 'Treasure Cruise', 'grave-2')
            seed(i31, 'Harmonized Trio', 'grave-31')
            bridgeTest.graveyardContainer = bridgeTest.graveyardDeck

            -- Same-name physical data elsewhere must not participate in the
            -- final-card correlation.
            local duplicate = BridgeTestCreateCard(
                'forge:2e629fa847ea4b1f9e3e1d61a75eeb31:duplicate',
                'Harmonized Trio', 'duplicate-trio')
            duplicate._inLibrary = false
            BridgeRecordLooseCardIdentity(
                'forge:2e629fa847ea4b1f9e3e1d61a75eeb31:duplicate',
                'duplicate-trio', 'forge-player-1', 'battlefield')

            -- A contained GUID may still be addressable as a Card proxy. It
            -- is not a top-level object until the collapse is visible.
            local rawGetLive = BridgeGetLiveObjectByGuid
            BridgeGetLiveObjectByGuid = function(guid)
                local object = rawGetLive(guid)
                if object ~= nil and object._graveyardLooseVisible == false then return nil end
                if object ~= nil and object.tag == 'Card' and object._inDeck == true then return nil end
                return object
            end
            ...
            BridgeTestSetEventQueue(
                {sequence=165, kind='tap_changed', seatId='forge-player-1',
                    sourceZone='battlefield', destinationZone='battlefield',
                    cardInstanceId='forge:2e629fa847ea4b1f9e3e1d61a75eeb31:20',
                    cardName='Swamp', tapped=true, forgeSequence=58},
                {sequence=166, kind='card_moved', seatId='forge-player-1',
                    sourceZone='graveyard', destinationZone='exile', cardInstanceId=i6,
                    cardName='Murderous Cut', forgeSequence=58},
                {sequence=167, kind='card_moved', seatId='forge-player-1',
                    sourceZone='graveyard', destinationZone='exile', cardInstanceId=i24,
                    cardName='Swamp', forgeSequence=58},
                {sequence=168, kind='card_moved', seatId='forge-player-1',
                    sourceZone='graveyard', destinationZone='exile', cardInstanceId=i2,
                    cardName='Treasure Cruise', forgeSequence=58},
                {sequence=169, kind='card_moved', seatId='forge-player-1',
                    sourceZone='graveyard', destinationZone='exile', cardInstanceId=i31,
                    cardName='Harmonized Trio', forgeSequence=58})
            local swamp = BridgeTestCreateCard(
                'forge:2e629fa847ea4b1f9e3e1d61a75eeb31:20', 'Swamp', 'swamp-20')
            swamp._inLibrary = false
            BridgeRecordLooseCardIdentity(
                'forge:2e629fa847ea4b1f9e3e1d61a75eeb31:20', 'swamp-20',
                'forge-player-1', 'battlefield')

            BridgeProcessEventQueue()
            local function advanceUntilTakeCount(expected)
                local guard = 0
                while bridgeTest.graveyardTakeDispatchCount < expected do
                    local progressed = BridgeTestFlushNextGraveyardTake()
                    if not progressed then progressed = BridgeTestFlushOneWait() end
                    if not progressed then error('serialized graveyard drain stopped before expected take') end
                    guard = guard + 1
                    if guard > 100 then error('serialized graveyard drain livelock') end
                end
            end
            advanceUntilTakeCount(3)
            BridgeTestFlushNextGraveyardTake()
            -- The third take callback retired the native Deck. The final
            -- loose Card is intentionally still undiscoverable here.
            BridgeTestFlushOneWait()
            dispatchBeforeReveal = bridgeTest.graveyardTakeDispatchCount
            finalStillContainedBeforeReveal = BridgeState.physicalContainerByInstanceId[i31] ~= nil
            BridgeTestRevealCollapsedGraveyardCard()
            local guard = 0
            while BridgeState.lastAppliedEventSequence < 169 do
                local progressed = BridgeTestFlushNextGraveyardTake()
                if not progressed then progressed = BridgeTestFlushOneWait() end
                if not progressed then error('serialized graveyard drain stopped before commit') end
                guard = guard + 1
                if guard > 200 then error('serialized graveyard drain commit livelock') end
            end

            finalMapping = BridgeState.physicalByInstanceId[i31]
            finalInverse = finalMapping and BridgeState.physicalInstanceIdByGuid[finalMapping] or nil
            finalSeat = finalMapping and BridgeState.physicalSeatByGuid[finalMapping] or nil
            finalZone = finalMapping and BridgeState.physicalZoneByGuid[finalMapping] or nil
            finalContainer = BridgeState.physicalContainerByInstanceId[i31]
            finalContainedInverse = BridgeState.physicalContainedInstanceIdByGuid['grave-31']
            graveyardEntries = BridgeTestArrayLength(bridgeTest.graveyardDeck.entries or {})
            graveyardContainer = bridgeTest.graveyardContainer ~= nil
            finalTakeDispatchCount = bridgeTest.graveyardTakeDispatchCount
            commitCount = BridgeTestCountLogToken('MUTATION_COMMIT')
            abortCount = BridgeTestCountLogToken('MUTATION_ABORT')
            local function journalStageCount(stage)
                local count = 0
                for _, entry in pairs(BridgeState.physicalMutationJournal or {}) do
                    if entry ~= nil and entry.stage == stage then count = count + 1 end
                end
                return count
            end
            topologyRebindCount = journalStageCount('TOPOLOGY_REBOUND')
            looseExtractionCount = journalStageCount('LOOSE_LOCATOR_RESOLVED')
            locatorFailureCount = journalStageCount('LOCATOR_RESOLVE_FAILED')
            staleDeckDispatchCount = BridgeTestCountLogToken('no contained mapping for card instance')
            finalApplied = BridgeState.lastAppliedEventSequence
            desync = BridgeState.desyncLatched == true
        ");

        Assert.Equal(3, lua.Globals.Get("dispatchBeforeReveal").Number);
        Assert.True(lua.Globals.Get("finalStillContainedBeforeReveal").Boolean);
        Assert.Equal(3, lua.Globals.Get("finalTakeDispatchCount").Number);
        Assert.Equal("grave-31", lua.Globals.Get("finalMapping").String);
        Assert.Equal("forge:2e629fa847ea4b1f9e3e1d61a75eeb31:31",
            lua.Globals.Get("finalInverse").String);
        Assert.Equal("forge-player-1", lua.Globals.Get("finalSeat").String);
        Assert.Equal("exile", lua.Globals.Get("finalZone").String);
        Assert.True(lua.Globals.Get("finalContainer").IsNil());
        Assert.True(lua.Globals.Get("finalContainedInverse").IsNil());
        Assert.Equal(0, lua.Globals.Get("graveyardEntries").Number);
        Assert.False(lua.Globals.Get("graveyardContainer").Boolean);
        Assert.Equal(169, lua.Globals.Get("finalApplied").Number, 0);
        Assert.Equal(1, lua.Globals.Get("commitCount").Number);
        Assert.Equal(0, lua.Globals.Get("abortCount").Number);
        Assert.True(lua.Globals.Get("topologyRebindCount").Number >= 1);
        Assert.True(lua.Globals.Get("looseExtractionCount").Number >= 1);
        Assert.Equal(0, lua.Globals.Get("locatorFailureCount").Number);
        Assert.Equal(0, lua.Globals.Get("staleDeckDispatchCount").Number);
        Assert.False(lua.Globals.Get("desync").Boolean, CapturedLogsTail(lua));
    }

    [Fact]
    public void GraveyardCollapseLeavesOneExactLooseCardForTheNextDecision()
    {
        var lua = NewProbe();
        ExecuteProbe(lua, @"
            BridgeTestInitAtomicHarness()
            BridgeState.eventSessionId = 'leave-one-session'
            BridgeState.eventSessionGeneration = 1
            BridgeState.physicalTransactionGeneration = 1
            BridgeState.lastAppliedEventSequence = 0
            bridgeTest.collapseGraveyardAtOne = true

            local i1 = 'forge:leave-one-session:1'
            local i2 = 'forge:leave-one-session:2'
            local i3 = 'forge:leave-one-session:3'
            local i4 = 'forge:leave-one-session:4'
            local function seed(instanceId, cardName, guid)
                BridgeState.cardNameByInstanceId[instanceId] = cardName
                local card = BridgeTestCreateCard(instanceId, cardName, guid)
                card._inDeck = true
            ...
            seed(i1, 'Harmonized Trio', 'leave-1')
            seed(i2, 'Swamp', 'leave-2')
            seed(i3, 'Harmonized Trio', 'leave-3')
            seed(i4, 'Treasure Cruise', 'leave-4')
            bridgeTest.graveyardContainer = bridgeTest.graveyardDeck
            pendingWaitCallbacks = {}
            function BridgeTestFlushOneWait()
                local callback = pendingWaitCallbacks[1]
            ...
            BridgeTestSetEventQueue(
                {sequence=1, kind='card_moved', seatId='forge-player-1', sourceZone='graveyard', destinationZone='exile', cardInstanceId=i1, cardName='Harmonized Trio', forgeSequence=1},
                {sequence=2, kind='card_moved', seatId='forge-player-1', sourceZone='graveyard', destinationZone='exile', cardInstanceId=i2, cardName='Swamp', forgeSequence=1},
                {sequence=3, kind='card_moved', seatId='forge-player-1', sourceZone='graveyard', destinationZone='exile', cardInstanceId=i3, cardName='Harmonized Trio', forgeSequence=1})
            BridgeProcessEventQueue()
            local guard = 0
            while BridgeState.lastAppliedEventSequence < 3 do
                local progressed = BridgeTestFlushNextGraveyardTake()
                if not progressed then progressed = BridgeTestFlushOneWait() end
                if not progressed then error('leave-one extraction stopped') end
                guard = guard + 1
                if guard > 100 then error('leave-one extraction livelock') end
            end
            while BridgeTestFlushOneWait() do end
            remaining = BridgeState.physicalByInstanceId[i4]
            remainingInverse = remaining and BridgeState.physicalInstanceIdByGuid[remaining] or nil
            remainingContainer = BridgeState.physicalContainerByInstanceId[i4]
            remainingContainedInverse = BridgeState.physicalContainedInstanceIdByGuid['leave-4']
            remainingSeat = remaining and BridgeState.physicalSeatByGuid[remaining] or nil
            remainingZone = remaining and BridgeState.physicalZoneByGuid[remaining] or nil
            BridgeTestSetEventQueue({sequence=4, kind='card_moved', seatId='forge-player-1', sourceZone='graveyard', destinationZone='exile', cardInstanceId=i4, cardName='Treasure Cruise', forgeSequence=2})
            BridgeProcessEventQueue()
            guard = 0
            while BridgeState.lastAppliedEventSequence < 4 do
                local progressed = BridgeTestFlushNextGraveyardTake()
                if not progressed then progressed = BridgeTestFlushOneWait() end
                if not progressed then error('later loose-card extraction stopped') end
                guard = guard + 1
                if guard > 100 then error('later loose-card extraction livelock') end
            end
            finalZone = BridgeState.physicalZoneByGuid['leave-4']
            finalContainer = bridgeTest.graveyardContainer ~= nil
            finalEntries = BridgeTestArrayLength(bridgeTest.graveyardDeck.entries or {})
            finalTakeCount = bridgeTest.graveyardTakeDispatchCount
            finalDesync = BridgeState.desyncLatched == true
        ");

        Assert.Equal("leave-4", lua.Globals.Get("remaining").String);
        Assert.Equal("forge:leave-one-session:4", lua.Globals.Get("remainingInverse").String);
        Assert.True(lua.Globals.Get("remainingContainer").IsNil());
        Assert.True(lua.Globals.Get("remainingContainedInverse").IsNil());
        Assert.Equal("forge-player-1", lua.Globals.Get("remainingSeat").String);
        Assert.Equal("graveyard", lua.Globals.Get("remainingZone").String);
        Assert.Equal("exile", lua.Globals.Get("finalZone").String);
        Assert.False(lua.Globals.Get("finalContainer").Boolean);
        Assert.Equal(0, lua.Globals.Get("finalEntries").Number);
        Assert.Equal(3, lua.Globals.Get("finalTakeCount").Number);
        Assert.False(lua.Globals.Get("finalDesync").Boolean, CapturedLogsTail(lua));
    }

    [Fact]
    public void TwoCardAndOneLooseCardGraveyardsUseExactPhysicalSources()
    {
        var lua = NewProbe();
        ExecuteProbe(lua, @"
            BridgeTestInitAtomicHarness()
            BridgeState.eventSessionId = 'boundary-session'
            BridgeState.eventSessionGeneration = 1
            BridgeState.physicalTransactionGeneration = 1
            BridgeState.lastAppliedEventSequence = 0
            bridgeTest.collapseGraveyardAtOne = true
            local iA = 'forge:boundary-session:1'
            local iB = 'forge:boundary-session:2'
            local function seedBoundary(instanceId, cardName, guid)
                BridgeState.cardNameByInstanceId[instanceId] = cardName
                local card = BridgeTestCreateCard(instanceId, cardName, guid)
                card._inDeck = true
                card._inLibrary = false
            ...
            seedBoundary(iA, 'Swamp', 'boundary-a')
            seedBoundary(iB, 'Harmonized Trio', 'boundary-b')
            bridgeTest.graveyardContainer = bridgeTest.graveyardDeck
            BridgeRecordContainedCardIdentity(iA, 'grave-deck', 'boundary-a', 'forge-player-1', 'graveyard', 'Swamp')
            BridgeRecordContainedCardIdentity(iB, 'grave-deck', 'boundary-b', 'forge-player-1', 'graveyard', 'Harmonized Trio')
            pendingWaitCallbacks = {}
            function BridgeTestFlushOneWait()
                local callback = pendingWaitCallbacks[1]
                if callback == nil then return false end
            ...
            local function drainUntil(sequence)
                local guard = 0
                while BridgeState.lastAppliedEventSequence < sequence do
                    local progressed = BridgeTestFlushNextGraveyardTake()
                    if not progressed then progressed = BridgeTestFlushOneWait() end
                    if not progressed then error('boundary extraction stopped applied=' .. tostring(BridgeState.lastAppliedEventSequence)
                        .. ' active=' .. tostring(BridgeState.graveyardExtractionActiveBySeatId['forge-player-1'])
                        .. ' settling=' .. tostring(BridgeState.graveyardExtractionTopologySettlingBySeatId['forge-player-1'])
                        .. ' desync=' .. tostring(BridgeState.desyncLatched)
                        .. ' reason=' .. tostring(desyncReason)) end
                    guard = guard + 1
                    if guard > 100 then error('boundary extraction livelock') end
                end
                while BridgeTestFlushOneWait() do end
            end
            BridgeTestSetEventQueue({sequence=1, kind='card_moved', seatId='forge-player-1', sourceZone='graveyard', destinationZone='exile', cardInstanceId=iA, cardName='Swamp', forgeSequence=1})
            BridgeProcessEventQueue()
            drainUntil(1)
            afterFirst = BridgeState.physicalByInstanceId[iB]
            afterFirstContainer = BridgeState.physicalContainerByInstanceId[iB]
            BridgeTestSetEventQueue({sequence=2, kind='card_moved', seatId='forge-player-1', sourceZone='graveyard', destinationZone='exile', cardInstanceId=iB, cardName='Harmonized Trio', forgeSequence=2})
            BridgeProcessEventQueue()
            drainUntil(2)

            local one = 'forge:boundary-session:one'
            BridgeState.cardNameByInstanceId[one] = 'Swamp'
            local loose = BridgeTestCreateCard(one, 'Swamp', 'boundary-one')
            loose._inLibrary = false
            BridgeRecordLooseCardIdentity(one, 'boundary-one', 'forge-player-1', 'graveyard')
            BridgeTestSetEventQueue({sequence=3, kind='card_moved', seatId='forge-player-1', sourceZone='graveyard', destinationZone='exile', cardInstanceId=one, cardName='Swamp', forgeSequence=3})
            BridgeProcessEventQueue()
            drainUntil(3)
            oneZone = BridgeState.physicalZoneByGuid['boundary-one']
            oneContainer = BridgeState.physicalContainerByInstanceId[one]
            takeCount = bridgeTest.graveyardTakeDispatchCount
            finalContainer = bridgeTest.graveyardContainer ~= nil
            finalEntries = BridgeTestArrayLength(bridgeTest.graveyardDeck.entries or {})
            finalDesync = BridgeState.desyncLatched == true
        ");

        Assert.Equal("boundary-b", lua.Globals.Get("afterFirst").String);
        Assert.True(lua.Globals.Get("afterFirstContainer").IsNil());
        Assert.Equal("exile", lua.Globals.Get("oneZone").String);
        Assert.True(lua.Globals.Get("oneContainer").IsNil());
        Assert.Equal(1, lua.Globals.Get("takeCount").Number);
        Assert.False(lua.Globals.Get("finalContainer").Boolean);
        Assert.Equal(0, lua.Globals.Get("finalEntries").Number);
        Assert.False(lua.Globals.Get("finalDesync").Boolean, CapturedLogsTail(lua));
    }
    ...
    [Fact]
    public void LateContainedCallbackCannotOverwritePublishedCollapsedLooseIdentity()
    {
        var lua = NewProbe();
        ExecuteProbe(lua, @"
            BridgeTestInitAtomicHarness()
            BridgeState.eventSessionId = 'stale-collapse-session'
            BridgeState.eventSessionGeneration = 1
            BridgeState.physicalTransactionGeneration = 1
            bridgeTest.collapseGraveyardAtOne = true
            local iA = 'forge:stale-collapse-session:1'
            local iB = 'forge:stale-collapse-session:2'
            local function seedStale(instanceId, cardName, guid)
                BridgeState.cardNameByInstanceId[instanceId] = cardName
                local card = BridgeTestCreateCard(instanceId, cardName, guid)
                card._inDeck = true
                card._inLibrary = false
            ...
            local cardA = seedStale(iA, 'Swamp', 'stale-a')
            local cardB = seedStale(iB, 'Harmonized Trio', 'stale-b')
            bridgeTest.graveyardContainer = bridgeTest.graveyardDeck
            BridgeRecordContainedCardIdentity(iA, 'grave-deck', 'stale-a', 'forge-player-1', 'graveyard', 'Swamp')
            BridgeRecordContainedCardIdentity(iB, 'grave-deck', 'stale-b', 'forge-player-1', 'graveyard', 'Harmonized Trio')
            callbackOk = nil
            callbackReason = nil
            BridgeTakeContainedCardByIdentity(iA, {x=0, y=0, z=0}, false, function(card, reason)
                callbackOk = card ~= nil
                callbackReason = tostring(reason or 'nil')
            end)
            -- The old Deck has collapsed while the callback is delayed. A
            -- newer topology publication owns the exact remaining Card.
            bridgeTest.graveyardDeck.entries = {}
            bridgeTest.graveyardContainer = nil
            bridgeTest.collapsedGraveyardDeck = true
            bridgeTest.collapsedGraveyardOldDeckGuid = 'grave-deck'
            bridgeTest.activeGraveyardDeckGuid = nil
            cardB._inDeck = false
            cardB._graveyardLooseVisible = true
            BridgeState.physicalTransactionGeneration = 2
            BridgeRecordLooseCardIdentity(iB, 'stale-b', 'forge-player-1', 'graveyard', true)
            BridgeTestFlushNextGraveyardTake()
            publishedLoose = BridgeState.physicalByInstanceId[iB]
            publishedInverse = BridgeState.physicalInstanceIdByGuid['stale-b']
            publishedContainer = BridgeState.physicalContainerByInstanceId[iB]
            oldContained = BridgeState.physicalContainerByInstanceId[iA]
            oldContainedInverse = BridgeState.physicalContainedInstanceIdByGuid['stale-a']
        ");

        Assert.False(lua.Globals.Get("callbackOk").Boolean);
        Assert.Contains("stale", lua.Globals.Get("callbackReason").String);
        Assert.Equal("stale-b", lua.Globals.Get("publishedLoose").String);
        Assert.Equal("forge:stale-collapse-session:2", lua.Globals.Get("publishedInverse").String);
        Assert.True(lua.Globals.Get("publishedContainer").IsNil());
        Assert.True(lua.Globals.Get("oldContained").IsNotNil());
        Assert.Equal("forge:stale-collapse-session:1", lua.Globals.Get("oldContainedInverse").String);
    }
    ...
    public void GraveyardMergeSettlesBeforeCommitThenDelveExtractsRecentlyMergedCardExactly()
    {
        ...
        ExecuteProbe(lua, @"
            ...
            -- Model TTS' native 2 -> 1 Deck collapse for the Delve drain.
            bridgeTest.collapseGraveyardAtOne = true
    ...
    public void FailedGraveyardFormationCanonicalizesEveryContainedLooseAlias()
    {
        ...
        ExecuteProbe(lua, @"
            BridgeTestInitAtomicHarness()
            BridgeState.cardNameByInstanceId[':incoming-a'] = 'Island'
            BridgeState.cardNameByInstanceId[':incoming-b'] = 'Island'
            BridgeState.cardNameByInstanceId[':incoming-c'] = 'Swamp'
            local incomingA = BridgeTestCreateCard(':incoming-a', 'Island', 'incoming-guid-a')
            local incomingB = BridgeTestCreateCard(':incoming-b', 'Island', 'incoming-guid-b')
            local incomingC = BridgeTestCreateCard(':incoming-c', 'Swamp', 'incoming-guid-c')
            incomingA._inLibrary = false
            incomingB._inLibrary = false
            incomingC._inLibrary = false
            incomingA._inDeck = false
            incomingB._inDeck = false
            incomingC._inDeck = false
            bridgeTest.graveyardContainer = bridgeTest.graveyardDeck
            bridgeTest.graveyardDeck.entries = {
                {instanceId=':incoming-a', name='Island', guid='incoming-guid-a'},
                {instanceId=':incoming-b', name='Island', guid='incoming-guid-b'},
                {instanceId=':incoming-c', name='Swamp', guid='incoming-guid-c'}
            }
            BridgeRecordLooseCardIdentity(':incoming-a', 'incoming-guid-a', 'forge-player-1', 'graveyard')
            BridgeRecordLooseCardIdentity(':incoming-b', 'incoming-guid-b', 'forge-player-1', 'graveyard')
            BridgeRecordLooseCardIdentity(':incoming-c', 'incoming-guid-c', 'forge-player-1', 'graveyard')
            BridgeAbortAtomicGraveyardMutation(nil, {
                seatId='forge-player-1',
                stagedPhysicalMoves={
                    {guid='incoming-guid-a', cardInstanceId=':incoming-a', seatId='forge-player-1'},
                    {guid='incoming-guid-b', cardInstanceId=':incoming-b', seatId='forge-player-1'},
                    {guid='incoming-guid-c', cardInstanceId=':incoming-c', seatId='forge-player-1'}
                },
                incomingInstanceIds={':incoming-a', ':incoming-b', ':incoming-c'}
            }, 'deliberately-interrupted-formation')
            local containedA = BridgeState.physicalContainerByInstanceId[':incoming-a']
            local containedB = BridgeState.physicalContainerByInstanceId[':incoming-b']
            local containedC = BridgeState.physicalContainerByInstanceId[':incoming-c']
            canonicalized = containedA ~= nil and containedB ~= nil and containedC ~= nil
                and containedA.cardGuid == 'incoming-guid-a'
                and containedB.cardGuid == 'incoming-guid-b'
                and containedC.cardGuid == 'incoming-guid-c'
                and containedA.zoneName == 'graveyard'
                and containedB.zoneName == 'graveyard'
                and containedC.zoneName == 'graveyard'
            looseMappingRetired = BridgeState.physicalByInstanceId[':incoming-a'] == nil
                and BridgeState.physicalByInstanceId[':incoming-b'] == nil
                and BridgeState.physicalByInstanceId[':incoming-c'] == nil
            looseObjectRetired = bridgeTest.cardsByGuid['incoming-guid-a'] == nil
                and bridgeTest.cardsByGuid['incoming-guid-b'] == nil
                and bridgeTest.cardsByGuid['incoming-guid-c'] == nil
            nativeRepresentationCount = BridgeTestGraveyardInstanceCount(':incoming-a')
                + BridgeTestGraveyardInstanceCount(':incoming-b')
                + BridgeTestGraveyardInstanceCount(':incoming-c')
            mutationFailed = BridgeTestCountLogToken('MUTATION_ABORT') >= 1
        ");

        Assert.True(lua.Globals.Get("canonicalized").Boolean, CapturedLogsTail(lua));
        Assert.True(lua.Globals.Get("looseMappingRetired").Boolean, CapturedLogsTail(lua));
        Assert.True(lua.Globals.Get("looseObjectRetired").Boolean, CapturedLogsTail(lua));
        Assert.Equal(3, lua.Globals.Get("nativeRepresentationCount").Number);
        Assert.True(lua.Globals.Get("mutationFailed").Boolean);
    }

    [Fact]
    public void StaleContainedGraveyardExtractionCallbackCannotMutateNewSession()
    {
        var lua = NewProbe();
        ExecuteProbe(lua, @"
            BridgeTestInitAtomicHarness()
            BridgeState.eventSessionId = 'old-contained-session'
            BridgeState.eventSessionGeneration = 3
            BridgeState.physicalTransactionGeneration = 7
            local oldInstanceId = 'forge:old-contained-session:41'
            local oldCard = BridgeTestCreateCard(oldInstanceId, 'Old graveyard card', 'old-contained-guid')
            oldCard._inLibrary = false
            oldCard._inDeck = true
            bridgeTest.graveyardContainer = bridgeTest.graveyardDeck
            bridgeTest.graveyardDeck.entries = {
                {instanceId=oldInstanceId, name='Old graveyard card', guid='old-contained-guid'}
            }
            BridgeRecordContainedCardIdentity(oldInstanceId, bridgeTest.graveyardDeck.getGUID(),
                'old-contained-guid', 'forge-player-1', 'graveyard', 'Old graveyard card')
            local staleCompletion = nil
            BridgeQueueGraveyardExtraction('forge-player-1', function(done)
                staleCompletion = done
            end, {
                cardInstanceId=oldInstanceId,
                deckGuid='grave-deck',
                physicalCompletion=function(ok, reason)
                    staleCompletionCalls = (staleCompletionCalls or 0) + 1
                end
            })
            oldQueueActive = BridgeState.graveyardExtractionActiveBySeatId['forge-player-1'] == true
            BridgeState.eventSessionId = 'new-contained-session'
            BridgeState.eventSessionGeneration = 4
            BridgeState.physicalTransactionGeneration = 8
            staleCompletion(true, nil)
            newSessionStillActive = BridgeState.eventSessionId == 'new-contained-session'
            newGenerationStillActive = BridgeState.physicalTransactionGeneration == 8
            oldQueueRetired = BridgeState.graveyardExtractionActiveBySeatId['forge-player-1'] ~= true
                and BridgeTestArrayLength(BridgeState.graveyardExtractionQueueBySeatId['forge-player-1'] or {}) == 0
            staleCompletionCalls = staleCompletionCalls or 0
            oldMappingStillContained = BridgeState.physicalContainerByInstanceId[oldInstanceId] ~= nil
                and BridgeState.physicalContainedInstanceIdByGuid['old-contained-guid'] == oldInstanceId
        ");

        Assert.True(lua.Globals.Get("oldQueueActive").Boolean);
        Assert.True(lua.Globals.Get("newSessionStillActive").Boolean);
        Assert.True(lua.Globals.Get("newGenerationStillActive").Boolean);
        Assert.True(lua.Globals.Get("oldQueueRetired").Boolean);
        Assert.Equal(0, lua.Globals.Get("staleCompletionCalls").Number);
        Assert.True(lua.Globals.Get("oldMappingStillContained").Boolean);
    }
        ...
        ExecuteProbe(lua, @"
            ...
            function BridgeTestInitAtomicHarness()
                ...
                bridgeTest = {
                    ...
                    collapseGraveyardAtOne = false,
                    collapsedGraveyardDeck = false,
                    collapsedGraveyardLooseVisible = true,
                    graveyardTakeDispatchCount = 0,
                    graveyardTakeDispatchInstanceIds = {}
                }
                ...
                bridgeTest.graveyardDeck = {
                    ...
                    getGUID = function()
                        ...
                        return bridgeTest.activeGraveyardDeckGuid or 'grave-deck'
                    end,
                    entries = {}
                }
                ...
                -- Model TTS' callback-based native Deck extraction. The Deck
                -- GUID is allowed to change as the pile is mutated, and the
                -- callback is flushed explicitly by the regression tests.
                bridgeTest.graveyardDeck.takeObject = function(options)
                    if bridgeTest.collapsedGraveyardDeck == true then
                        error('graveyard takeObject was called after native Deck collapsed')
                    end
                    local requestedGuid = options and options.guid or nil
                    local requestedIndex = options and options.index or nil
                    local selectedIndex = nil
                    bridgeTest.forEachArrayEntry(bridgeTest.graveyardDeck.entries, function(entry)
                        if selectedIndex == nil
                            and ((requestedGuid ~= nil and tostring(entry.guid) == tostring(requestedGuid))
                                or (requestedGuid == nil and tonumber(entry.index) == tonumber(requestedIndex))) then
                            for index = 1, BridgeTestArrayLength(bridgeTest.graveyardDeck.entries) do
                                if bridgeTest.graveyardDeck.entries[index] == entry then
                                    selectedIndex = index
                                    break
                                end
                            end
                    ...
                    if selectedIndex == nil then error('graveyard takeObject could not find exact contained GUID') end
                    local entry = bridgeTest.graveyardDeck.entries[selectedIndex]
                    bridgeTest.graveyardTakeDispatchCount = bridgeTest.graveyardTakeDispatchCount + 1
                    table.insert(bridgeTest.graveyardTakeDispatchInstanceIds, requestedGuid)
                    local oldDeckGuid = bridgeTest.activeGraveyardDeckGuid or 'grave-deck'
                    local entryCount = BridgeTestArrayLength(bridgeTest.graveyardDeck.entries)
                    for shift = selectedIndex, entryCount - 1 do
                        bridgeTest.graveyardDeck.entries[shift] = bridgeTest.graveyardDeck.entries[shift + 1]
                    end
                    bridgeTest.graveyardDeck.entries[entryCount] = nil
                    local card = bridgeTest.cardsByGuid[entry.guid]
                    if card == nil then error('graveyard takeObject has no physical card for exact GUID') end
                    card._inDeck = false
                    card._inLibrary = false
                    card._inHand = false
                    card._lastPosition = options and options.position or nil
                    bridgeTest.pendingGraveyardTakes = bridgeTest.pendingGraveyardTakes or {}
                    table.insert(bridgeTest.pendingGraveyardTakes, {
                        callback = options and options.callback_function or nil,
                        card = card, oldDeckGuid = oldDeckGuid, entry = entry
                    })
                    bridgeTest.activeGraveyardDeckGuid = oldDeckGuid .. '-next'
                    bridgeTest.graveyardDeckAliases = bridgeTest.graveyardDeckAliases or {}
                    bridgeTest.graveyardDeckAliases[oldDeckGuid] = true
                    if BridgeTestArrayLength(bridgeTest.graveyardDeck.entries) == 0 then
                        bridgeTest.graveyardContainer = nil
                    elseif bridgeTest.collapseGraveyardAtOne == true
                        and BridgeTestArrayLength(bridgeTest.graveyardDeck.entries) == 1 then
                        -- TTS no longer exposes a one-card native Deck. Keep
                        -- the exact remaining Card object alive, but retire
                        -- the old Deck and optionally delay its visibility so
                        -- the serialized worker must reobserve the type
                        -- transition rather than racing the take callback.
                        local remaining = bridgeTest.graveyardDeck.entries[1]
                        local remainingCard = remaining ~= nil
                            and bridgeTest.cardsByGuid[remaining.guid] or nil
                        if remainingCard == nil then
                            error('collapsed graveyard Deck has no exact remaining Card')
                        end
                        remainingCard._inDeck = false
                        remainingCard._inLibrary = false
                        remainingCard._inHand = false
                        remainingCard._lastPosition = bridgeTest.graveyardDeck._lastPosition
                        remainingCard._graveyardLooseVisible = bridgeTest.collapsedGraveyardLooseVisible == true
                        bridgeTest.graveyardDeck.entries = {}
                        bridgeTest.collapsedGraveyardDeck = true
                        bridgeTest.collapsedGraveyardLooseGuid = remaining.guid
                        bridgeTest.collapsedGraveyardOldDeckGuid = oldDeckGuid
                        bridgeTest.activeGraveyardDeckGuid = nil
                        bridgeTest.graveyardContainer = nil
                    end
                end

                function BridgeTestFlushNextGraveyardTake()
                    local pending = bridgeTest.pendingGraveyardTakes
                    if pending == nil or pending[1] == nil then return false end
                    local item = pending[1]
                    local pendingCount = BridgeTestArrayLength(pending)
                    for shift = 1, pendingCount - 1 do pending[shift] = pending[shift + 1] end
                    pending[pendingCount] = nil
                    if item.callback ~= nil then item.callback(item.card) end
                    return true
                end

                function BridgeTestRevealCollapsedGraveyardCard()
                    if bridgeTest.collapsedGraveyardLooseGuid == nil then return false end
                    local card = bridgeTest.cardsByGuid[bridgeTest.collapsedGraveyardLooseGuid]
                    if card == nil then return false end
                    card._graveyardLooseVisible = true
                    bridgeTest.collapsedGraveyardLooseVisible = true
                    return true
                end
                ...
                bridgeTest.graveyardDeck.putObject = function(object, position)
                    ...
                    bridgeTest.graveyardContainer = bridgeTest.graveyardDeck
                    bridgeTest.collapsedGraveyardDeck = false
                    bridgeTest.collapsedGraveyardLooseGuid = nil
                    bridgeTest.collapsedGraveyardOldDeckGuid = nil
                ...
                function BridgeTestCreateCard(instanceId, cardName, guid)
                    ...
                    card.destruct = function()
                        card._destructed = true
                        card._inDeck = false
                        card._inLibrary = false
                        card._inHand = false
                        bridgeTest.cardsByGuid[card._guid] = nil
                    end
                ...
                function BridgeTestSeedExistingDeck(...)
                    ...
                    bridgeTest.collapsedGraveyardLooseGuid = nil
                    bridgeTest.collapsedGraveyardOldDeckGuid = nil
                    bridgeTest.activeGraveyardDeckGuid = 'grave-deck'
                ...
                function getObjectFromGUID(guid)
                    ...
                    if bridgeTest.collapsedGraveyardDeck == true
                        and (guid == 'grave-deck'
                            or guid == bridgeTest.activeGraveyardDeckGuid
                            or guid == bridgeTest.collapsedGraveyardOldDeckGuid
                            or (bridgeTest.graveyardDeckAliases ~= nil
                                and bridgeTest.graveyardDeckAliases[guid] == true)) then
                        return nil
                    end
                    if guid == 'grave-deck' or guid == bridgeTest.activeGraveyardDeckGuid
                        or (bridgeTest.graveyardDeckAliases ~= nil and bridgeTest.graveyardDeckAliases[guid] == true) then
                        return bridgeTest.graveyardDeck
                    end
                ...
                function getAllObjects()
                    ...
                    bridgeTest.forEachArrayEntry(bridgeTest.allCards or {}, function(card)
                        if card._destructed ~= true and card._inDeck ~= true
                            and card._inLibrary ~= true and card._inHand ~= true
                            and (card._graveyardLooseVisible ~= false) then
